...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDetail> handleResourceNotFound(ResourceNotFoundException ex,
        ...
        ErrorDetail errorDetail = new ErrorDetail(LocalDateTime.now(), 404, "Not Found", ex.getMessage(),
                request.getRequestURI());
        return ResponseEntity.status(404).body(errorDetail);
    }

    @ExceptionHandler(InsufficientBalanceException.class)
    public ResponseEntity<ErrorDetail> handleInsufficentBalance(InsufficientBalanceException ex,
            HttpServletRequest request) {
        ErrorDetail errorDetail = new ErrorDetail(LocalDateTime.now(), 422, "Insufficient Balance", ex.getMessage(),
                request.getRequestURI());
    ...
    public ResponseEntity<ErrorDetail> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
            HttpServletRequest request) {
        ErrorDetail errorDetail = new ErrorDetail(LocalDateTime.now(), 400, "Bad Request", "Invalid input data",
                request.getRequestURI());
        return ResponseEntity.status(400).body(errorDetail);
    }

    @ExceptionHandler(DuplicateResourceException.class)
    public ResponseEntity<ErrorDetail> handleDuplicateResource(DuplicateResourceException ex,
            HttpServletRequest request) {
        ErrorDetail errorDetail = new ErrorDetail(LocalDateTime.now(), 409, "Duplicate Resource", ex.getMessage(),
                request.getRequestURI());
    ...
    public ResponseEntity<ErrorDetail> handleGenericException(Exception ex, HttpServletRequest request) {
        ...
        ErrorDetail errorDetail = new ErrorDetail(LocalDateTime.now(), 500, "Internal Server Error",
